...
class LTX2MemoryEfficientSageAttentionPatch(io.ComfyNode):
    # Используем кэш на уровне модуля, а не класса
    _sage_cache = {}
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            category="KJNodes/ltxv",
            description="EXPERIMENTAL! Activates custom sageattention to reduce peak VRAM usage. Works with modern sageattention versions.",
            is_experimental=True,
            inputs=[
                io.Model.Input("model"),
                io.Combo.Input(
                    "attention_mode",
                    options=["auto", "sageattn", "disabled"],
                    default="auto",
                    tooltip="'auto' - use sageattn if available, 'sageattn' - force sageattn, 'disabled' - disable optimization"
                ),
            ],
            outputs=[
                io.Model.Output(display_name="model"),
    ...
    @classmethod
    def _get_sage_info(cls):
        """Получение информации о SageAttention без изменения атрибутов класса"""
        cache_key = "sage_info"

        # Проверяем кэш
        if cache_key in cls._sage_cache:
            return cls._sage_cache[cache_key]
        ...
        try:
            ...
            info['module'] = sageattention
            info['available'] = True

            # Доступные функции
            funcs = [f for f in dir(sageattention) if not f.startswith('_')]
            info['functions'] = funcs
            info['has_sageattn'] = hasattr(sageattention, 'sageattn')

            # Определяем архитектуру GPU
            if torch.cuda.is_available():
                capability = torch.cuda.get_device_capability(0)
                arch = f"sm{capability[0]}{capability[1]}"
                info['arch'] = arch
                print(f"✅ SageAttention обнаружен для архитектуры {arch}")
                print(f"   Доступные функции: {funcs}")
            else:
                print("⚠️ CUDA не доступна")

        except ImportError as e:
            print(f"⚠️ SageAttention не установлен: {e}")

        # Сохраняем в кэш
        cls._sage_cache[cache_key] = info
        return info

    @classmethod
    def execute(cls, model, attention_mode="auto") -> io.NodeOutput:
        # Получаем информацию без изменения атрибутов класса
        sage_info = cls._get_sage_info()

        # Проверка режима
        if attention_mode == "disabled":
            print("ℹ️ SageAttention отключен пользователем")
            return io.NodeOutput(model)

        if not sage_info['available']:
            print("⚠️ SageAttention не доступен, пропускаем патч")
            return io.NodeOutput(model)

        if sage_info['arch'] is None:
            print("⚠️ Не удалось определить архитектуру CUDA, пропускаем патч")
            return io.NodeOutput(model)

        # Проверяем наличие функции sageattn
        if not sage_info['has_sageattn']:
            print("⚠️ Функция sageattn не найдена в модуле")
            print(f"   Доступные функции: {sage_info['functions']}")
            return io.NodeOutput(model)

        model_clone = model.clone()
        diffusion_model = model_clone.get_model_object("diffusion_model")

        print(f"🔧 Применение SageAttention патча к {len(diffusion_model.transformer_blocks)} блокам")
        print(f"   Режим: {attention_mode}")
        print(f"   Архитектура: {sage_info['arch']}")

        # ВАЖНО: Импортируем функцию здесь, где она будет доступна
        from comfy.ldm.lightricks.model import apply_rotary_emb

        # Создаем замыкание с захватом модуля, архитектуры и функции apply_rotary_emb
        sage_module = sage_info['module']
        arch = sage_info['arch']

        def create_sageattn_forward():
            # Эти переменные будут захвачены замыканием
            local_sage_module = sage_module
            local_arch = arch
            local_attention_mode = attention_mode
            local_apply_rotary_emb = apply_rotary_emb  # захватываем функцию

            def sageattn_forward(self, x, context=None, mask=None, pe=None, k_pe=None, transformer_options={}):
                dtype = x.dtype
                ...
                q = self.q_norm(q)
                if pe is not None:
                    q = local_apply_rotary_emb(q, pe)  # используем захваченную функцию

                # key
                k = self.to_k(context)
                k = self.k_norm(k)
                if pe is not None:
                    k = local_apply_rotary_emb(k, pe if k_pe is None else k_pe)  # используем захваченную функцию
                ...
                q = q.view(batch_size, seq_len, self.heads, head_dim)
                k = k.view(batch_size, k.shape[1], self.heads, head_dim)
                v = v.view(batch_size, v.shape[1], self.heads, head_dim)

                # Транспонируем для sageattn (ожидает batch, heads, seq_len, head_dim)
                ...
                try:
                    # Используем sageattn
                    if local_attention_mode == "sageattn" or local_attention_mode == "auto":
                        out = local_sage_module.sageattn(q, k, v)
                    else:
                        # Fallback на стандартный attention
                        scale = head_dim ** -0.5
                    ...
                    print(f"⚠️ SageAttention error: {e}, using fallback")
                    # Fallback на стандартный attention
                    scale = head_dim ** -0.5
                    out = torch.nn.functional.scaled_dot_product_attention(q, k, v, scale=scale)
                    out = out.permute(0, 2, 1, 3).contiguous()
            ...
            return sageattn_forward

        # Применяем патч ко всем блокам
        sage_forward = create_sageattn_forward()

        for idx, block in enumerate(diffusion_model.transformer_blocks):
            model_clone.add_object_patch(
                f"diffusion_model.transformer_blocks.{idx}.attn1.forward",
                sage_forward.__get__(block.attn1, block.attn1.__class__)
            )

        print(f"✅ SageAttention патч успешно применен")
        return io.NodeOutput(model_clone)
